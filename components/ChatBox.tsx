...
const ChatBox: React.FC<ChatBoxProps> = ({ roomId, playerId, playerName, agentId }) => {
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-[100] transition-all duration-300 ease-in-out flex flex-col pointer-events-auto ${minimized ? 'w-48' : 'w-80 h-[450px]'}`}>
            {/* Header */}
            <div
                className="bg-[#0F1923] border border-white/10 p-4 flex items-center justify-between cursor-pointer rounded-t-lg shadow-2xl"
                onClick={() => setMinimized(!minimized)}
            >
                <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-[#FF4655] animate-pulse" />
                    <span className="text-[10px] uppercase font-black tracking-widest text-white/80">Tactical Comms</span>
                </div>
            ...
            {!minimized && (
                <>
                    ...
                    <div
                        ref={scrollRef}
                        className="flex-1 bg-[#0F1923]/95 backdrop-blur-xl border-x border-white/10 p-4 space-y-4 overflow-y-auto scrollbar-hide"
                    >
                        {messages.length === 0 ? (
                    ...
                    <form
                        onSubmit={handleSend}
                        className="bg-[#0F1923] border border-white/10 p-3 rounded-b-lg shadow-2xl flex gap-2"
                    >
                        <input
                            type="text"
                            value={text}
                            ...
                            className="flex-1 bg-white/5 border border-white/10 px-3 py-2 text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-[#FF4655]/50 transition-colors"
                        />
                        <button
                            type="submit"
                            disabled={!text.trim()}
                            className="bg-[#FF4655] hover:bg-white text-white hover:text-black p-2 transition-all disabled:opacity-30 disabled:hover:bg-[#FF4655] disabled:hover:text-white"
                        >
